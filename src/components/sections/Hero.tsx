...
export function Hero() {
  ...
  return (
    <section id="top" className="relative w-full overflow-hidden bg-earth-deep">

      {/* ===== MOBILE: imagem em cima, conteúdo embaixo ===== */}
      <div className="md:hidden flex flex-col min-h-[100svh]">
        {/* Imagem completa no topo */}
        <div className="relative w-full aspect-[4/3]">
          ...
          />
          {/* Gradiente no topo para contraste do logo */}
          <div className="absolute top-0 left-0 w-full h-40 bg-gradient-to-b from-earth-deep/70 to-transparent" />
          {/* Gradiente na base da imagem para transição suave */}
          <div className="absolute bottom-0 left-0 w-full h-28 bg-gradient-to-t from-earth-deep to-transparent" />
          {/* Logo sobre a imagem */}
          <motion.div
            initial={reduce ? undefined : { opacity: 0, y: 24 }}
            animate={reduce ? undefined : { opacity: 1, y: 0 }}
            transition={{ duration: 1.4, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
            className="absolute -bottom-[13px] left-0 w-full flex justify-center"
          >
            <Image
              ...
              width={320}
              height={128}
              className="h-36 w-auto brightness-0 invert opacity-95"
              priority
            />
          </motion.div>
        </div>
        ...
        {/* Rodapé branco divisor */}
        <div className="w-full h-6 bg-off-white" />
      </div>

      {/* ===== DESKTOP: layout original com imagem full background ===== */}
      <div className="hidden md:block relative h-[100svh] min-h-[680px]">
        {/* Background */}
        ...
        {/* Conteúdo */}
        <div className="relative mx-auto flex h-full max-w-[1440px] flex-col justify-between px-12 pb-40 pt-36 lg:pb-52">
          <motion.div
            initial={reduce ? undefined : { opacity: 0, y: 24 }}
            animate={reduce ? undefined : { opacity: 1, y: 0 }}
            transition={{ duration: 1.4, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
          ...
          >
            <Image
              ...
              width={320}
              height={128}
              className="h-32 w-auto brightness-0 invert opacity-95 lg:h-36"
              priority
            />
          </motion.div>
